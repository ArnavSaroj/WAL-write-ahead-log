...
> This is a **learning project** — a basic implementation meant to understand how
> a WAL actually works. It is not a production library.
...
```mermaid
flowchart TD
    A[Client / main] -->|append| B[WAL::append<br/>build header + payload<br/>write to stream buffer]
    B -->|flush| C[WAL::flush<br/>stream.flush then _commit fsync]
    C --> D[(test.wal<br/>append-only file)]
    D -->|on open| E[WAL::recover<br/>scan, verify CRC,<br/>stop at first bad record,<br/>restore next_lsn]
    F[fault injector corrupt.h<br/>flip bit / truncate / smash size] -.->|corrupts| D
```
